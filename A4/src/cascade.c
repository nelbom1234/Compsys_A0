...
csc_file_t* csc_parse_file(const char* sourcefile, const char* destination)
{
    ...
    memcpy(&res->filename, sourcefile, sizeof(sourcefile));
    ...
    if (destination == NULL) {
        //This is exactly where the code dies. absolutely no idea why
        return NULL;
    }
...
//same as get_peers_list, but command is 2 instead of 1
...
void *upload_thread(void* arg) {
    ...
    while (1) {
        int listenfd = open_clientfd(my_ip, my_port);
        int connection = Accept(listenfd, clientaddr, &clientlen);
        Rio_readinitb(&rio, listenfd);

        struct ClientRequest client_request;
        Rio_readlineb(&rio, msg_buf, MAXLINE);
        memcpy(&client_request, msg_buf, sizeof(struct ClientRequest));
        struct ClientResponseHeader response;
        response.error[0] = 0;
        if (strcmp(client_request.protocol, "CASCADE1")) {
            response.error[0] = 4;
            response.length = 0;
            memcpy(msg_buf, &response, PEER_RESPONSE_HEADER_SIZE);
            rio_writen(connection, msg_buf, PEER_RESPONSE_HEADER_SIZE);
            printf("faulty request protocol");
            close(listenfd);
            close(connection);
            continue;
        }
        else if (client_request.reserved != 0) {
            response.error[0] = 4;
            response.length = 0;
            memcpy(msg_buf, &response, PEER_RESPONSE_HEADER_SIZE);
            rio_writen(connection, msg_buf, PEER_RESPONSE_HEADER_SIZE);
            printf("reserved bytes not 0");
            close(listenfd);
            close(connection);
            continue;
        }
        uint64_t blocknum = be64toh(client_request.block_num);
        csc_file_t *rfile = malloc(sizeof(csc_file_t));
        //to check if we broke out of for loop ir it just terminated without finding
        rfile->filename = "not found";
        for (size_t i = 0; i < ((upload_info_t*) arg)->size; i++) {
            if (((upload_info_t*)arg)->files[i].targethash == client_request.hash) {
                rfile = &((upload_info_t*)arg)->files[i];
                break;
            }
        }
        if (strcmp(rfile->filename, "not found") == 0) {
            response.error[0] = 1;
            response.length = 0;
            memcpy(msg_buf, &response, PEER_RESPONSE_HEADER_SIZE);
            rio_writen(connection, msg_buf, PEER_RESPONSE_HEADER_SIZE);
        ...
        if (rfile->blockcount < blocknum) {
            response.error[0] = 3;
            response.length = 0;
            memcpy(msg_buf, &response, PEER_RESPONSE_HEADER_SIZE);
            rio_writen(connection, msg_buf, PEER_RESPONSE_HEADER_SIZE);
            printf("block number is larger than blocks in file");
            close(listenfd);
            close(connection);
            continue;
        }
        else if (rfile->blocks[blocknum].completed == 0) {
            response.error[0] = 2;
            response.length = 0;
            memcpy(msg_buf, &response, PEER_RESPONSE_HEADER_SIZE);
            rio_writen(connection, msg_buf, PEER_RESPONSE_HEADER_SIZE);
            printf("block has not been downloaded yet");
            close(listenfd);
            close(connection);
            continue;
        }
        uint64_t msglen;
        if (blocknum == rfile->blockcount-1) {
            msglen = rfile->trailblocksize;
        }
        else msglen = rfile->blocksize;

        char * blockdata = calloc(msglen+1, sizeof(char));
        blockdata[msglen] = '\0';
        FILE* fp = Fopen(rfile->filename, "r");
        if (fp == 0) {
            printf("Failed to open destination: %s\n", rfile->filename);
            free(blockdata);
            Close(connection);
        ...
        fseek(fp, rfile->blocks[blocknum].offset, SEEK_SET);
        Fread(&blockdata, sizeof(char), msglen, fp);
        Fclose(fp);

        memcpy(&response.length, msglen, sizeof(uint64_t));
        memcpy(msg_buf, &response, PEER_RESPONSE_HEADER_SIZE);
        strcpy(msg_buf+PEER_RESPONSE_HEADER_SIZE, blockdata);
        rio_writen(connection, msg_buf, PEER_RESPONSE_HEADER_SIZE+msglen);
...
//keep looping through our cascade files to download blocks as they show up
void *download_thread(void* arg) {
    while(1) {
        for (size_t i = 0; i < ((upload_info_t*)arg)->size; i++) {
            download_only_peer(((upload_info_t*)arg)->files[i].filename);
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    upload_info_t* ui = malloc(sizeof(upload_info_t));
    ui->files = malloc(sizeof(csc_file_t)*casc_count);
    for (int i = 0; i < casc_count; i++) {
        ui->files[i] = *csc_parse_file(cascade_files[i], NULL);
        printf("are we here \n");
        subscribe(ui->files[i].targethash);
    }
    ui->size = casc_count;
    pthread_t tid_upload;
    pthread_t tid_download;
    pthread_create(&tid_upload, NULL, &upload_thread, ui);
    pthread_create(&tid_download, NULL, &download_thread, ui);
