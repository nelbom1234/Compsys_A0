...
int main(int argc, char* argv[]) {
    ...
    while (!stop) { // SIMULATION BEGINS - for each cycle:
        ...
        //if a bool is 1 then that must be the major_op and thus we can infer the size of the instruction
        ...
        val ins_size = add(add(add(add(add(add(add(add(add(add(add(add(
            ...
            use_if(is_imm_arithmetic, from_int(6))),
            use_if(is_reg_movq, from_int(2))),
            use_if(is_imm_movq, from_int(6))),
            use_if(is_reg_movq_mem, from_int(2))),
            use_if(is_imm_movq_mem, from_int(6))),
            use_if(is_cflow, from_int(6))),
            use_if(is_leaq2, from_int(2))),
            use_if(is_leaq3, from_int(3))),
            use_if(is_leaq6, from_int(6))),
            use_if(is_leaq7, from_int(7))),
